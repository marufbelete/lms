...
import { CourseService,UserService,RoleService, ExerciseService, LessonService } from "../service/index.service";
import { getByIdSchema } from "../validation/common.validation";
import { roleToUserSchema, courseToUserSchema} from "../validation/user.validation";
...
deleteRoleFromUser  : async (req:Request, res:Response, next:NextFunction) => {
  try {
...
getUserCoursesWithProgress : async (req:Request, res:Response, next:NextFunction) => {
  try {
    const {id}= req.params
...
getUserCoursesInfo : async (req:Request, res:Response, next:NextFunction) => {
  try {
    const {id}= req.params
    const {error}=getByIdSchema.validate({
      id
...
getUserCourseWithProgress : async (req:Request, res:Response, next:NextFunction) => {
  try {
...
getUserCourseInfo : async (req:Request, res:Response, next:NextFunction) => {
  try {
    const {id:user_id,course_id}= req.params
    const {error}=courseToUserSchema.validate({
      user_id,course_id
