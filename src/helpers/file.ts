...
export const uploadImage=multer({ storage: fileStorage, fileFilter: sanitizeImage})
...
export const saveImage=async(file:Express.Multer.File,folder:string)=>{
    ...
    const params = {
      ...
      .toFormat(imagetype as keyof FormatEnum | AvailableFormatInfo)
      .toBuffer(),
...
export const saveImages=async(files:Express.Multer.File[],folder:string)=>{
  const imgurl=[]
...
export function sanitizeImage(req:Request, file:Express.Multer.File, cb:CallableFunction) { 
