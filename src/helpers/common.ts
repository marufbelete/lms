...
const lessons = data.lesson_users?.map((lessonUser) => {
  const completedExercises = lessonUser.exercise_users?.filter((exercise) => exercise.dataValues.is_completed);
  return {
...
export const paginate = (filter:IncludeOptions, { page, pageSize}:IPaginate) => {
if(page<1){
